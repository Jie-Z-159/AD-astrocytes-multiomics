...
for (name in names(seurat_objects)) {
  ...
  data_sex <- left_join(data_sex, sig_sex, by = "Group")
  ...
  data_sex <- data_sex %>%
    ...
    mutate(sig_color = case_when(
      p_value >= 0.05 ~ "grey",
      p_value < 0.05 & ADNC == "Low" & mean_FAscore > mean_FAscore[ADNC == "Not AD"] ~ "high",
      p_value < 0.05 & ADNC == "Low" ~ "low",
      p_value < 0.05 & ADNC == "Not AD" & mean_FAscore > mean_FAscore[ADNC == "Low"] ~ "high",
      p_value < 0.05 & ADNC == "Not AD" ~ "low"
    ))
  
  data_sex$sig_color <- factor(data_sex$sig_color, levels = c("grey", "low", "high"))
  
  plot_sex <- ggplot(data_sex, aes(x = ADNC, y = Group)) +
    geom_point(aes(size = mean_FAscore, fill = sig_color),
               shape = 21, color = "lightgray") +
    geom_text(aes(label = round(mean_FAscore, 3)), color = "black", size = 3) +
    geom_text(
      data = subset(data_sex, ADNC == "Low"),
      aes(label = p_label),
      color = "black",
      size = 3,
      nudge_x = 0.65
    ...
    scale_fill_manual(name = "Significance",
                      values = c("grey" = "grey",
                                 ...
                                 "high" = "#fbb4ae")) +
    theme_minimal() +
    labs(x = "Pathology stage", y = "Sex", title = paste0(name, " - by Sex"))
  
  #======================#
  # 2. age
  #======================#
  data_age <- data_to_plot %>%
    group_by(Age_death, ADNC) %>%
    summarise(mean_FAscore = mean(FAscore1, na.rm = TRUE), .groups = "drop") %>%
    filter(Age_death %in% c("78 to 89 years old", "90+ years old")) %>%
    group_by(Age_death) %>%
    filter(n_distinct(ADNC) == 2) %>%
    mutate(Group = as.character(Age_death))
  
  sig_age <- data_to_plot %>%
    group_by(Age_death) %>%
    filter(n_distinct(ADNC) == 2) %>%
    group_by(Group = Age_death) %>%
    summarise(
      p_value = wilcox.test(FAscore1[ADNC == "Not AD"],
                            FAscore1[ADNC == "Low"],
                            paired = FALSE)$p.value,
  ...
  data_age <- left_join(data_age, sig_age, by = "Group")
  ...
  data_age$sig_color <- factor(data_age$sig_color, levels = c("grey", "low", "high"))
  
  plot_age <- ggplot(data_age, aes(x = ADNC, y = Group)) +
    geom_point(aes(size = mean_FAscore, fill = sig_color),
               shape = 21, color = "lightgray") +
    geom_text(aes(label = round(mean_FAscore, 3)), color = "black", size = 3) +
    geom_text(
      data = subset(data_age, ADNC == "Low"),
      aes(label = p_label),
      color = "black",
      size = 3,
      nudge_x = 0.65
    ...
    scale_fill_manual(name = "Significance",
                      values = c("grey" = "grey",
                                 ...
                                 "high" = "#fbb4ae")) +
    theme_minimal() +
    labs(x = "Pathology stage", y = "Age at Death", title = paste0(name, " - by Age"))
  
 
  plots_list[[name]] <- plot_sex + plot_age + plot_layout(ncol = 2)
}

final_plots <- wrap_plots(plots_list, ncol = 1)
final_plots
